...
| `AUTH0_PWA_CLIENT_ID`             | variable | pwa-ehpk.yml          | Client ID of the **SPA** Auth0 application registered for the PWA / glasses bundle.           |
| `APPLE_CERT_P12`                  | secret   | mac-bundle.yml        | Base64-encoded `.p12` of the Developer ID Application cert + private key. Tag pushes only.    |
| `APPLE_CERT_PASSWORD`             | secret   | mac-bundle.yml        | Password used when exporting the `.p12` from Keychain Access.                                 |
| `APP_STORE_CONNECT_API_KEY_ID`    | secret   | mac-bundle.yml        | Key ID from App Store Connect → Users and Access → Keys → Integrations.                       |
| `APP_STORE_CONNECT_API_ISSUER_ID` | secret   | mac-bundle.yml        | Issuer ID shown on the same App Store Connect page.                                           |
| `APP_STORE_CONNECT_API_KEY`       | secret   | mac-bundle.yml        | Raw `.p8` contents (including `-----BEGIN/END PRIVATE KEY-----` lines). Used by `notarytool`. |
...
The deployed server reads its config from environment variables set
by the orchestrator. The deploy topology — including the env template
and secret list (Auth0 tenant, Postgres password, LLM provider creds,
mnemo URL) — lives in the [kleos repo](https://github.com/tiagodeoliveira/kleos).
...
Deployment is owned by the [kleos repo](https://github.com/tiagodeoliveira/kleos),
which pulls the auris-server image from GHCR alongside mnemo and runs
both behind nginx (TLS terminator, Cloudflare-fronted) with scheduled
backups. See kleos's README for the runbook. The server's `:7331`
carries auth tokens as a WS query parameter, so plain HTTP exposure
is a credential-leak risk — kleos handles the TLS termination.
