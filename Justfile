...
server-run:
    ...
    AUTH0_API_AUDIENCE=https://kleos.tiago.tools \
...
# Run the PWA dev server (port 5173). Server URL + any other VITE_*
# vars are read from packages/pwa/.env and .env.local at startup — no
# shell injection here, so a .env.local pointing at the prod server
# (wss://auris.tiago.tools) wins for prototype-mode QR testing. Use
# `just server-run` if you want the dev bundle to talk to a local
# server and set VITE_SERVER_URL=ws://localhost:7331 in .env.local.
pwa-dev:
    pnpm -F @auris/pwa dev

# Run the PWA dev server + the EvenHub simulator pointed at it.
# Same env-from-files behavior as `pwa-dev`.
pwa-sim:
    pnpm -F @auris/pwa dev:sim

# Build the PWA bundle and pack it into an `.ehpk` ready to upload to
# the Even Hub developer portal. Output: packages/pwa/auris.ehpk.
#
# Delegates to the package's own `pack` script so the recipe stays
# in sync with whatever the PWA's build does — today that's
# whitelist-sync → vite build → evenhub pack, but the recipe doesn't
# care; it just kicks the script.
pwa-ehpk:
    pnpm -F @auris/pwa run pack
    @echo ""
    @echo "Built: packages/pwa/auris.ehpk"
...
mac-run:
    cd packages/mac && \
    AURIS_SERVER_URL=wss://auris.tiago.tools \
    AUTH0_DOMAIN=dev-jrva0wzk3qkdxcar.us.auth0.com \
    AUTH0_MAC_CLIENT_ID=YDK0XoDAIRhp2uORlfk8TijQkcqRzjsi \
    AUTH0_API_AUDIENCE=https://kleos.tiago.tools \
    swift run

# Build, sign, notarize, and staple a release-ready Auris.app
# locally. Output: packages/mac/Auris-<version>.zip ready to hand to
# anyone (or to your other Mac) with no Gatekeeper friction.
#
# Requires the local prerequisites documented in
# scripts/bundle-signed.sh (Developer ID cert in keychain +
# `xcrun notarytool store-credentials AURIS_NOTARY_PROFILE`).
#
# For a faster iteration loop (sign without notarizing), run
# `SKIP_NOTARIZE=1 just mac-bundle-signed` — the resulting bundle
# still needs right-click → Open on first launch but skips the
# Apple round-trip.
mac-bundle-signed:
    AURIS_SERVER_URL=wss://auris.tiago.tools \
    AUTH0_DOMAIN=dev-jrva0wzk3qkdxcar.us.auth0.com \
    ...
    bash packages/mac/scripts/bundle-signed.sh
...
# Publish an OTA (EAS Update) from your laptop. Mirrors what
# .github/workflows/mobile-update.yml does in CI, just from your
# machine instead of GitHub's runners — no build minutes consumed,
# ~15s end-to-end.
#
# `--environment` is what makes the bundle pick up EAS-stored env
# vars (Auth0, server URL, etc.); without it the OTA inlines empty
# strings (see commit fdea461). The branch name matches the
# environment name by project convention (production/preview/
# development).
#
# Footgun: bundles your LOCAL working tree, including .env.local and
# uncommitted changes. For a clean-room publish with a paper trail,
# push to main and let CI do it instead.
#
# Usage:
#   just mobile-ota                # publish to production
#   just mobile-ota preview        # publish to preview channel
mobile-ota BRANCH="production":
    cd packages/mobile && \
    eas update --branch {{BRANCH}} --environment {{BRANCH}} --auto --non-interactive
...
ios-run:
    cd packages/mobile && \
    EXPO_PUBLIC_SERVER_URL=wss://auris.tiago.tools \
    ...
    EXPO_PUBLIC_AUTH0_API_AUDIENCE=https://kleos.tiago.tools \
...
llm-smoke-bedrock description="Q1 budget review for helix product launch":
    AURIS_LLM_BACKGROUND_PROVIDER=bedrock AURIS_LLM_BACKGROUND_MODEL_ID=us.anthropic.claude-sonnet-4-7-20251015-v1:0 cargo run -p auris-server --example llm_smoke -- "{{description}}"

# Smoke-test against OpenAI specifically.
llm-smoke-openai description="Q1 budget review for helix product launch":
    AURIS_LLM_BACKGROUND_PROVIDER=openai AURIS_LLM_BACKGROUND_MODEL_ID=gpt-4o cargo run -p auris-server --example llm_smoke -- "{{description}}"
...
llm-smoke-anthropic description="Q1 budget review for helix product launch":
    AURIS_LLM_BACKGROUND_PROVIDER=anthropic AURIS_LLM_BACKGROUND_MODEL_ID=claude-opus-4-7 cargo run -p auris-server --example llm_smoke -- "{{description}}"

# Run the env-gated LLM integration test (provider selected via AURIS_LLM_BACKGROUND_PROVIDER + AURIS_LLM_BACKGROUND_MODEL_ID).
llm-integration:
