...
docker-compose.yml   Local-dev Postgres only (production deploy lives in the kleos repo).
```
...
- **LLM pools** — each pool picks its own provider + model:
  `AURIS_LLM_CHAT_PROVIDER` / `AURIS_LLM_CHAT_MODEL_ID` for the
  chat agent loop, `AURIS_LLM_BACKGROUND_PROVIDER` /
  `AURIS_LLM_BACKGROUND_MODEL_ID` for summary/moment/artifact/
  wrap-up. Both are required; provider is `bedrock` | `openai` |
  `anthropic`. Plus provider-specific keys.
...
The single-VM deployment stack — Postgres + auris-server + mnemo-server

- nginx (TLS) + scheduled backups — lives in a separate repo:
  [`kleos`](https://github.com/tiagodeoliveira/kleos). This repo owns
  only the auris source and image; kleos pulls pinned tags from GHCR
  and runs them alongside mnemo. The server image is published by
  `.github/workflows/server-image.yml` on every push to main and on tags.
