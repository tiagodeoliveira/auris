...
export type Intent =
  | {
       ...
       * (Moderate, matching historical behavior). */
...
export type Event =
  ...
  | {
      /** Direct response to `mint_pair_code`. Shown as-is by the
       * requesting surface; never broadcast to the user's other
       * sessions (the code is sensitive). */
      type: "pair_code_minted";
      code: string;
      /** ISO-8601 expiry. */
      expires_at: string;
    }
  | {
      /** Fan-out signal: the user's paired-devices set changed
       * (a device was redeemed or revoked). Carries no payload —
       * subscribers re-fetch via `GET /pair/devices`. */
      type: "paired_devices_changed";
    };
