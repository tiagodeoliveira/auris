...
export interface MeetingDetail extends MeetingSummary {
  ...
  /** Raw per-pool usage rows ("background" / "chat"). Empty for
   * meetings finalized before the pool split; older server builds
   * omit the field — treat absent as empty. */
   ...
   *   - "success" → done (zero items is valid); no banner
...
export class MeetingsApi {
  ...
  /// Mirrors the PWA/Mac clients. 204 on success; the server validates
