...
import { loadSettings, makeStorage, getOrCreateDeviceId } from "./storage";
...
describe("storage", () => {
  ...
  test("loadSettings reads stored values", async () => {
    ...
    bridge.storage["mc.glassesModes"] = JSON.stringify({ transcript: false });

    const s = await loadSettings(bridge, {});
    expect(s.serverToken).toBe("tok");
    expect(s.lastMetadata).toEqual({ project: "helix" });
    expect(s.glassesModes).toEqual({ transcript: false });
  });
  ...
  test("getOrCreateDeviceId generates + persists on first call", async () => {
    const id = await getOrCreateDeviceId(bridge);
    expect(id).toMatch(/^[0-9a-f-]{36}$/);
    // Persisted to the bridge KV so the next session reuses it.
    expect(bridge.storage["mc.deviceId"]).toBe(id);
  });

  test("getOrCreateDeviceId returns the persisted id on subsequent calls", async () => {
    bridge.storage["mc.deviceId"] = "stable-existing-id";
    const id = await getOrCreateDeviceId(bridge);
    expect(id).toBe("stable-existing-id");
    // Did not overwrite with a fresh uuid.
    expect(bridge.storage["mc.deviceId"]).toBe("stable-existing-id");
  });

  test("loadSettings filters non-boolean values out of glassesModes", async () => {
    // Defensive: cross-version localStorage rot or tampering could
    // land a string/number where a boolean is expected. parseRecord
    // drops bad entries rather than throwing, so the rest of the
    // app sees a clean Record<string, boolean>.
    bridge.storage["mc.glassesModes"] = JSON.stringify({
      transcript: false,
      highlights: "no",
      summary: 0,
      actions: true,
    ...
    const s = await loadSettings(bridge, {});
    expect(s.glassesModes).toEqual({ transcript: false, actions: true });
  });
});
