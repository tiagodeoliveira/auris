...
import { makeCloseButton, makeReloadButton } from "./modal-chrome";
...
export function mountMeetingsModal(parent: HTMLElement, store: Store, auth: AuthBundle): void {
  ...
  // Title block — Bebas Neue display with a mono caption that
  // reflects the current row count so the user has context the
  // moment the modal paints. The caption ref escapes the closure so
  // renderList() can update it as data arrives.
  const titleBlock = document.createElement("div");
  titleBlock.className = "meetings-title-block";
  const title = document.createElement("h2");
  title.className = "settings-title";
  title.textContent = "Meetings";
  const caption = document.createElement("div");
  caption.className = "display-caption";
  caption.textContent = "── tap a row to open."; // initial; renderList() refines
  titleBlock.append(title, caption);

  const reloadBtn = makeReloadButton(() => void reloadList());
  const closeBtn = makeCloseButton(() => store.update({ meetingsModalOpen: false }));
  header.append(backBtn, titleBlock, reloadBtn, closeBtn);
  ...
  /// Delete a meeting. Optimistic local remove + revert on failure
  /// (mirrors the Mac client's pattern). Mobile-friendly tap target;
  /// destructive confirm via window.confirm so it works without a
  /// custom dialog stack.
  async function handleDelete(id: string): Promise<void> {
    ...
    try {
      ...
      meetings = [...meetings.slice(0, idx), removed, ...meetings.slice(idx)];
      renderList();
      window.alert(`Couldn't delete meeting: ${errorMessage(e)}`);
    }
  }

  /// Rename a finished meeting by setting its `title` tag. Optimistic
  /// local update of both the open detail and the matching list row,
  /// with revert + alert on failure (mirrors `handleDelete`). `head`
  /// is the live `<h3>` so we can repaint it without a full rebuild.
  async function handleRename(
    ...
    if (!newTitle || newTitle === pickDetailTitle(detail)) return; // no-op / unchanged
    const api = makeApi();
    if (!api) return;
    const summary = meetings.find((m) => m.id === detail.id);
    const prevDetailTitle = detail.metadata.title;
    const prevSummaryTitle = summary?.metadata.title;
    detail.metadata.title = newTitle;
    if (summary) summary.metadata.title = newTitle;
    head.textContent = newTitle;
    renderList();
    try {
      await api.rename(detail.id, newTitle);
    } catch (e) {
      restoreTitle(detail.metadata, prevDetailTitle);
      if (summary) restoreTitle(summary.metadata, prevSummaryTitle);
      head.textContent = pickDetailTitle(detail);
      renderList();
      window.alert(`Couldn't rename meeting: ${errorMessage(e)}`);
    }
  }
  ...
  function renderList(): void {
    ...
    if (meetings.length === 0) {
      ...
      empty.textContent = "── tap Compose to capture one.";
      listPane.appendChild(empty);
      caption.textContent = "── no meetings yet.";
      return;
    }
    // Caption tracks the row count once the list has data. Singular
    // form for 1, plural otherwise; "most recent first" is informational
    // since the modal sorts server-order (newest first).
    caption.textContent =
      meetings.length === 1
        ? "1 MEETING · MOST RECENT FIRST"
        : `${meetings.length} MEETINGS · MOST RECENT FIRST`;
    ...
    for (const m of meetings) {
      ...
      // Row layout: tap area (title + sub) on the left, dedicated
      // delete button on the right. Wrapping div keeps the two
      // <button>s as valid siblings (button-in-button is invalid
      // HTML) and lets the border-bottom span the full row.
      const wrap = document.createElement("div");
      wrap.className = "meetings-row-wrap";
      ...
      del.className = "meetings-row-delete";
  ...
  function buildDetailView(detail: MeetingDetail): HTMLElement {
    ...
    // Rename affordance — swaps the title for an inline input seeded
    // with the current name. Enter / blur commits, Esc cancels. The
    // commit path optimistically updates the title and reverts on a
    // server error (see handleRename).
    const renameBtn = document.createElement("button");
    renameBtn.className = "btn-ghost meetings-detail-rename";
    renameBtn.setAttribute("aria-label", "Rename meeting");
    renameBtn.title = "Rename this meeting";
    renameBtn.textContent = "✎";
    renameBtn.addEventListener("click", () => {
      const input = document.createElement("input");
      input.type = "text";
      input.className = "meetings-detail-title-input";
      input.value = pickDetailTitle(detail);
      input.maxLength = 200;
      ...
      const settle = (commit: boolean): void => {
        ...
        if (commit) void handleRename(detail, value, head);
      };
    ...
    // Wrap-up extractor status banner. The extractor generates
    // actions + open_questions from the full transcript on
    // stop_meeting; before this banner, any failure (LLM timeout,
    // quota exhaustion, network blip) was silent — users couldn't
    // tell "no actions spoken" apart from "extractor crashed."
    //
    // We only render the banner for the two states that need user
    // feedback: `running` (still in flight — show a hint so they
    // know to refresh) and `failed` (errored — show with a retry
    // option). `success` and `null` (legacy) render nothing.
    const wrapUp = detail.wrap_up_status;
    if (wrapUp === "running" || wrapUp === "failed") {
      const banner = document.createElement("div");
      banner.className =
        wrapUp === "failed"
          ? "meetings-wrap-up-banner meetings-wrap-up-banner-failed"
          : "meetings-wrap-up-banner meetings-wrap-up-banner-running";
      banner.textContent =
        wrapUp === "failed"
  ...
  function buildItemRow(mode: string, item: Item): HTMLElement {
    ...
    if (mode === "chat") {
      ...
      // Attachment cue on user bubbles that rode screenshot(s) — the
      // server persists the ids in meta, so it shows in history too.
      ...
      if (role === "user" && Array.isArray(attachmentIds) && attachmentIds.length > 0) {
        ...
        badge.setAttribute("aria-label", `${n} image attachment${n > 1 ? "s" : ""}`);
    ...
    body.textContent = mode === "assist" ? prefixWithAssistGlyph(item.text, meta) : item.text;
  ...
  function formatItemMeta(mode: string, meta: Record<string, unknown> | null | undefined): string {
    ...
    if (mode === "assist") {
      const t = (meta.type as string)?.toUpperCase() ?? "";
      const c = typeof meta.confidence === "number" ? ` · ${Math.round(meta.confidence)}%` : "";
      return t ? `${t}${c}` : "";
    }
    return "";
  }

  /** Mirrors `buildBody`'s prefix in items-mirror.ts for assist
   * items so the finalized view matches the live overlay's at-a-
   * glance type chip. */
  function prefixWithAssistGlyph(
    text: string,
...
/// Restore a previously-captured `title` tag onto a metadata record.
/// When the prior value was absent (the meeting had no user/LLM title),
/// the key is removed so `pickDetailTitle` falls back to the
/// description/placeholder — rather than leaving a stale optimistic
/// title behind after a failed rename.
export function restoreTitle(metadata: Record<string, string>, prev: string | undefined): void {
  if (prev === undefined) {
...
// Re-exported so existing importers (and the modal's own test) keep
// resolving `pickDetailTitle` from this module after the helper moved
// to `meeting-format.ts`.
export { pickDetailTitle };
