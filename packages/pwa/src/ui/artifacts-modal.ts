...
export function mountArtifactsModal(parent: HTMLElement, store: Store, auth: AuthBundle): void {
  ...
  // Title block — Bebas Neue + mono caption. The caption reflects
  // the current library size; render() updates it whenever the
  // library list changes.
  const titleBlock = document.createElement("div");
  titleBlock.className = "artifacts-title-block";
  const title = document.createElement("h2");
  title.className = "settings-title";
  title.textContent = "Artifacts";
  const caption = document.createElement("div");
  caption.className = "display-caption";
  caption.textContent = "── upload to give meetings extra context.";
  ...
  const reloadBtn = makeReloadButton(() => void reload());
  const closeBtn = makeCloseButton(close);
  // Header carries only the title + corner reload/close icons,
  // matching the meetings / quick-asks / settings modals. Putting
  // the Upload CTA in the header alongside the icons made it the
  // fourth element on the row, so on mobile widths it wrapped and
  // pushed the icons to a second line below the title. Upload now
  // lives in the body as a primary-action row directly under the
  // header chrome.
  header.append(titleBlock, reloadBtn, closeBtn);
  ...
  // Upload row — full-width primary action that opens the file
  // picker. `fileInput` is hidden but lives next to the button so
  // the change handler stays scoped to this DOM block.
  const uploadRow = document.createElement("div");
  uploadRow.className = "artifacts-upload-row";
  ...
  function render(): void {
    body.innerHTML = "";
    // Caption tracks the library size. Empty states get an em-dash
    // hint instead of a count; everything else reads as "N ARTIFACT[S]…".
    if (library.length === 0) {
      caption.textContent = listLoading
        ? "── loading the library…"
        : "── upload to give meetings extra context.";
    } else {
      caption.textContent =
        library.length === 1
          ? "1 ARTIFACT IN THE LIBRARY"
          : `${library.length} ARTIFACTS IN THE LIBRARY`;
    }
    if (uploading) {
    ...
    if (library.length === 0 && !listError) {
      ...
      empty.textContent = "── upload a document or image to give meeting agents context.";
