...
async fn seed_dev_user_and_meeting() -> (String, String) {
    let pool = storage::open_pool().await.expect("open pool");
    let dev_user = storage::users::upsert_user_by_auth0_sub(
        &pool,
        "dev|local",
        Some("dev@local"),
        Some("Local Dev"),
...
async fn seed_meeting_for_foreign_user() -> String {
    ...
    let foreign = storage::users::upsert_user_by_auth0_sub(
        &pool,
...
async fn upload_happy_path() {
    ...
    let pool = storage::open_pool().await.unwrap();
    let row = storage::chat_attachments::get_chat_attachment(&pool, &id)
        .await
        .unwrap()
        .expect("row exists");
...
/// Drain the connect-time events (Snapshot + synthetic quick_asks
/// ItemsUpdate broadcast), send start_meeting, drain through the
/// handshake until `meeting_state_changed` arrives, and return the
/// server-allocated meeting_id. Resilient to extra pre-intent events
/// the harness might add — only stops on the type it cares about.
async fn start_meeting_via_ws(ws: &mut common::Ws) -> String {
    // Drain the two events that fire on every connect:
    //   1. Snapshot (direct send before the broadcast subscribe)
    //   2. ItemsUpdate for quick_asks (synthetic post-snapshot
    //      replay from ws/control.rs `run_control_socket`)
    let _ = common::next_event(ws, Duration::from_secs(2)).await;
    let _ = common::next_event(ws, Duration::from_secs(2)).await;

    common::send_intent(ws, serde_json::json!({"type":"start_meeting"})).await;
    let e1 = loop {
        let ev = common::next_event(ws, Duration::from_secs(2)).await;
        if ev["type"] == "meeting_state_changed" {
            break ev;
        }
        // Tolerate any other interleaved events (status heartbeats,
        // stale items_update) — the harness only asserts on the
        // meeting handshake.
    };
    ...
    let meeting_id = e1["meeting_id"]
        ...
        .to_string();
    // Drain the rest of the start-meeting handshake.
    ...
    let _ = common::next_event(ws, Duration::from_secs(2)).await; // assist_sensitivity_changed
    meeting_id
}
...
async fn chat_with_cross_tenant_attachment_emits_forbidden() {
    ...
    let pool = storage::open_pool().await.expect("open pool");
    let foreign_user = storage::users::upsert_user_by_auth0_sub(
        &pool,
        &format!("foreign|{}", uuid::Uuid::new_v4()),
        None,
        None,
    ...
    let dir = storage::data_dir().expect("data_dir");
    ...
    tokio::fs::write(&abs, b"\x89PNG\r\n\x1a\nfake")
        .await
        .expect("write png");
    storage::chat_attachments::insert_chat_attachment(
        &pool,
        &att_id,
        &foreign_meeting_id,
        &foreign_user.id,
