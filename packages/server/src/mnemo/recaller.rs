...
use crate::contract::{Event, MeetingState, PriorContextSummary};
...
/// Local state for the recaller task — separate from `ServerState`.
/// Tracks just enough to decide when to fire a fresh recall.
#[derive(Default)]
struct RecallerLocal {
    meeting_active: bool,
    /// Project we last recalled with. `None` = recalled with no project.
    /// Compared against the latest metadata to decide whether to re-fire.
...
async fn recaller_loop(
    ...
    let mut local = RecallerLocal::default();
    loop {
        match rx.recv().await {
            Ok(Event::MeetingStateChanged {
                ...
                trigger_recall(&client, &state, &events_tx, project);
            }
            Ok(Event::MeetingStateChanged {
                ...
                let _ = events_tx.send(Event::PriorContextChanged {
                    summary: PriorContextSummary::default(),
                });
            }
            Ok(Event::MetadataChanged { .. }) => {
                if !local.meeting_active {
                    continue; // pre-meeting edits don't trigger recall
                }
                let project = read_project(&state).await;
...
async fn read_project(state: &Arc<Mutex<ServerState>>) -> Option<String> {
    state
        .lock()
        .await
        .metadata_clone()
...
fn trigger_recall(
    ...
    tokio::spawn(async move {
        match client.recall(&params).await {
            Ok(ctx) => {
                if ctx.is_empty() {
                    debug!("mnemo recall returned no records");
                } else {
                    info!(
                ...
                state.lock().await.set_recalled_context(Some(ctx));
                let _ = events_tx.send(Event::PriorContextChanged { summary });
            }
            Err(e) => {
