...
//! goes Active, and stores the result on `SessionRegistry` so the LLM
//! summarizers can include it as prior context in their prompts.
//!
//! Lane note: the recaller subscribes to the **fan-out broadcast**,
//! not the durable queue, on purpose. A `Lagged` skip here costs one
//! prompt its prior-context enrichment — recoverable and invisible —
//! whereas the durable queue is reserved for events whose loss is
//! permanent (transcript, items, mnemo session lifecycle).
//!
//! Lifecycle:
...
use crate::protocol::{Event, MeetingState, PriorContextSummary, UserEvent};
            ...
            Ok(envelope) => {
                ...
                match envelope.event {
                    ...
                    Event::MeetingStateChanged {
                        ...
                        // Don't call set_recalled_context here — the field
                        // lives inside MeetingRuntime, and StopMeeting has
                        // already dropped it. The PriorContextChanged
                        // broadcast below is what clears wire state for
                        // clients.
...
async fn read_project(state: &Arc<Mutex<SessionRegistry>>, user_id: &str) -> Option<String> {
    state
        .lock()
        .await
        .user(user_id)
...
fn trigger_recall(
    ...
    tokio::spawn(async move {
        match client.recall(&user_id, &params).await {
            Ok(ctx) => {
                if ctx.is_empty() {
