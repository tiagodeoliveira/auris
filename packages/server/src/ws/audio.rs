...
use crate::ws::{auth_failed_response, WsAuthParams};
...
async fn run_stt_socket(
    ...
    // Dictation sessions have no meeting runtime to drain; cancel doubles
    // as the graceful-drain signal here. Permanent by design — unlike the
    // meeting pipeline in `control.rs`, this is NOT wired to a runtime drain.
    let drain_placeholder = provider_cancel.clone();
    ...
    loop {
        tokio::select! {
            ...
            evt = events_rx.recv() => {
                match evt {
                    // clippy wants the inner match folded into match guards,
                    // but our arms .await on send_json which match guards
                    // don't permit. Keep the nested shape.
