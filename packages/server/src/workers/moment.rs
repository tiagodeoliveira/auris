        ...
        loop {
            match rx.recv().await {
                Ok(req) => {
                    ...
                    let bus = bus.clone();
                    tokio::spawn(async move {
                        if let Err(e) = process_one(&db, &llm, &agent_kick_tx, &bus, &req).await {
                            warn!(error = ?e, moment_id = %req.moment_id, "moment summary failed");
                            let _ = crate::storage::moments::update_moment_summary(
                                &db,
                                &req.moment_id,
                                None,
                                "failed",
...
async fn process_one(
    db: &sqlx::PgPool,
    llm: &Arc<LlmClient>,
    agent_kick_tx: &tokio::sync::broadcast::Sender<crate::agent::AgentKick>,
    ...
    let row: Option<(Option<String>, Option<String>)> =
        sqlx::query_as("SELECT note, asset_path FROM moments WHERE id = $1")
            ...
            .fetch_optional(db)
            .await?;
    let (note, asset_path) = row.unwrap_or((None, None));
    ...
    crate::storage::moments::update_moment_summary(
        db,
        &req.moment_id,
        Some(&summary_trimmed),
        "done",
    )
    .await?;
    ...
    let _ = agent_kick_tx.send(crate::agent::AgentKick {
        user_id: req.user_id.clone(),
        reason: crate::agent::AgentKickReason::MomentSummarized {
    ...
    // Wire-broadcast the same payload (fan-out only by classification;
    // MomentSummarized isn't durable — the mnemo pusher intentionally
    // ignores it, and clients fall through today). A future "moment
    // ready" toast or list refresh would slot in here.
    bus.emit(
        req.user_id.clone(),
        crate::protocol::Event::MomentSummarized {
            moment_id: req.moment_id.clone(),
...
async fn read_transcript_window(meeting_id: &str, moment_t_ms: i64, window_ms: i64) -> Vec<Item> {
    let all = match crate::storage::persistence_loop::read_transcription(meeting_id).await {
        Ok(items) => items,
        Err(e) => {
