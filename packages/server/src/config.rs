...
/// Read an env var as `u64`, returning `default` if unset, empty,
/// or not parseable. Used for tunable timeouts / thresholds /
/// limits — e.g. `AURIS_SONIOX_RECONNECT_BASE_MS`. A malformed
/// value silently falls back to the default rather than failing
/// boot, on the principle that an unparseable knob shouldn't take
/// the service down; the warn surfaces the typo without blocking.
pub fn var_u64_or(key: &str, default: u64) -> u64 {
    match var_opt(key) {
        None => default,
        Some(v) => match v.parse() {
            Ok(n) => n,
            Err(_) => {
                tracing::warn!(key, value = %v, default, "env var not a valid u64; using default");
                default
            }
        },
    }
