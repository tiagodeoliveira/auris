...
// `import AurisContract` and get the full v1 type surface.
//
// Wire format is binary protobuf over WebSocket binary frames:
//   let bytes = try intent.serializedData()       // → Data
//   let parsed = try Auris_V1_Intent(serializedData: bytes)
//
// SwiftProtobuf prefixes types with the proto package, so
// `auris.v1.Intent` becomes `Auris_V1_Intent`. Callers can
// typealias for shorter local names if they prefer.
...
public let AurisContractProtocolVersion: UInt32 = 1
