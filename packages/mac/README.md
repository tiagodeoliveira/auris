...
```bash
just mac-bundle-signed
```

Builds a universal release binary, assembles `Auris.app`, signs
Sparkle.framework + the main bundle with your Developer ID cert and
`Auris.entitlements`, submits to `notarytool`, staples the ticket,
and re-zips. Output: `packages/mac/Auris-<version>.zip` ready to
hand out with zero Gatekeeper friction.

For iteration that skips the 1–10 min notarization round-trip
(produces a signed-but-not-stapled bundle; first launch still needs
right-click → Open):

```bash
SKIP_NOTARIZE=1 just mac-bundle-signed
```

Prerequisites: a "Developer ID Application" cert in your login
keychain and a notary profile stored via
`xcrun notarytool store-credentials AURIS_NOTARY_PROFILE …`. See
the header of `scripts/bundle-signed.sh` for details.
