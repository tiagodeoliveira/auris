...
extension Notification.Name {
    /// Posted by `AppModel.showOverlayWindow()` when the AppKit
    /// window list doesn't yet contain a "Meeting" window (rare —
    /// the SwiftUI `Window(id:)` scene is eager, so this only fires
    /// on a brand-new launch before the scene finishes materializing).
    /// `MenuBarContent` observes this on appear and calls
    /// `openWindow(id: "meeting-overlay")`.
    static let aurisRequestShowOverlay = Notification.Name("AurisRequestShowOverlay")
}
...
final class AppModel {
    ...
    /// Observes main-thread responsiveness and logs hangs to OSLog
    /// (subsystem `com.auris.mac`, category `watchdog`). Diagnostic
    /// only — captures the "incredibly slow → beachball" freeze with
    /// the last-handled event + memory at hang onset, since the cause
    /// isn't reproducible from static analysis. Not `@Observable`
    /// state; no view reads it (a `let`, so `@Observable` skips it).
    ...
    /// Active meeting's assist sensitivity. Updated by `snapshot`
    /// + `assist_sensitivity_changed` events; sent in the next
    /// `start_meeting` intent and via `set_assist_sensitivity` for
    /// mid-meeting flips. Default `moderate` matches the server's
    /// pre-feature behavior.
    private(set) var assistSensitivity: AssistSensitivity = .moderate
    ...
    /// True between `start_meeting` send and the matching
    /// `meeting_state_changed { active }` echo. Lets the
    /// `meeting_state_changed` handler distinguish "this Mac
    /// started it" from "another surface started it." Cleared by
    /// either the echo or a 3s safety timer — whichever fires
    /// first. A stale `true` would wrongly classify a subsequent
    /// remote-start as local, so the timer is load-bearing.
    private(set) var isLocallyStartingMeeting: Bool = false
    ...
    /// Meeting metadata chips. Server auto-extracts from the meeting
    /// description on start_meeting and emits `metadata_changed`; this
    /// view also reflects manual edits via `set_metadata`. StartMeeting
    /// intentionally omits metadata so the server preserves any chips
    /// already set.
    ...
    init() {
        // Start the main-thread hang watchdog first so it covers the
        // whole app lifetime, including connect/registration.
        watchdog.start()
    ...
    /// Stable per-install device id, persisted in UserDefaults so it
    /// survives reconnects and app restarts. Sent on every
    /// `register_device` so the server reuses this Mac's identity (and
    /// keeps its audio-source binding) instead of minting a fresh id
    /// each reconnect — the bug that silently unbound the mic when the
    /// socket dropped.
    private static func stableDeviceId() -> String {
        let key = "auris.deviceId"
    ...
    /// True when THIS Mac is locally capturing audio for a meeting.
    /// Narrower than `hasActiveMeeting` — false when a meeting is
    /// running but driven by another client (phone, PWA). Used for
    /// the Live/Muted indicator and post-local-start success checks.
    ...
    /// True when there's an active meeting for this user — anywhere.
    /// Includes meetings driven by another client (phone, PWA) where
    /// this Mac is just a control surface. Most UI gates (menu bar,
    /// overlay mode, action buttons) want THIS signal, not the
    /// narrower `isMeetingActive`.
    ...
    /// and the server reports no active meeting *anywhere* for this
    /// user. The `currentMeetingId` clause is the cross-surface
    /// guard (single-active-meeting invariant): if a meeting is
    /// already running on the PWA or phone, our local audio is idle
    /// but the server would reject our `start_meeting` anyway.
    ...
    func startMeeting(description: String? = nil) async {
        ...
        // Mark the start as locally-initiated so the
        // `meeting_state_changed { active }` echo knows to force-show
        // the overlay (Rule 5: local start always shows, regardless
        // of `overlayAutoShow`). Schedule a 3s safety clear in case
        // the echo never arrives — a lingering true would misclassify
        // a later remote-start as local.
        isLocallyStartingMeeting = true
        scheduleLocallyStartingClear()
        ...
        do {
            ...
            print("[AppModel] start_meeting sent (description=\(description ?? "nil"), source=\(ownDevice?.id ?? "nil"), sensitivity=\(assistSensitivity.rawValue))")
        } catch {
            print("[AppModel] start_meeting send failed: \(error)")
            audioStreamer.stop()
            audioCapture.stop()
            meetingStartedAt = nil
            isLocallyStartingMeeting = false
        }
    }

    /// Safety-clear `isLocallyStartingMeeting` after ~3s in case the
    /// server's echo never arrives. The active-echo path also clears
    /// it; whichever fires first wins.
    private func scheduleLocallyStartingClear() {
        Task { [weak self] in
            try? await Task.sleep(for: .seconds(3))
            self?.isLocallyStartingMeeting = false
        }
    }

    /// Bring the floating meeting overlay to the front. The
    /// `Window("Meeting", id: "meeting-overlay")` scene registers a
    /// real NSWindow at launch; closing it via AppKit's `.close()`
    /// keeps the handle in `NSApp.windows`, so `makeKeyAndOrderFront`
    /// re-shows the existing instance without needing
    /// `@Environment(\.openWindow)` (which isn't reachable outside
    /// of a View). If no matching window is found — only happens on
    /// the very first show before the scene materializes — post a
    /// notification so the menu-bar / overlay views (whichever is
    /// alive) call `openWindow(id:)` on our behalf.
    func showOverlayWindow() {
        ...
        if !shown {
            NotificationCenter.default.post(
                name: .aurisRequestShowOverlay, object: nil
            )
        }
        NSApp.activate(ignoringOtherApps: true)
    }

    /// Update assist sensitivity. Two code paths:
    ///   - Compose surface (no active meeting): set the local value
    ///     only. It rides on the next `start_meeting` intent.
    ///   - Mid-meeting: fire `SetAssistSensitivityIntent`. The
    ///     server flips its runtime field + persists the column +
    ///     broadcasts `assist_sensitivity_changed`; our own
    ///     dispatch arm mirrors the value back into AppModel so
    ///     other surfaces stay in sync without a special-case here.
    func setAssistSensitivity(_ value: AssistSensitivity) async {
        assistSensitivity = value
        guard isMeetingActive else { return }
        do {
            try await webSocket.send(intent: SetAssistSensitivityIntent(value: value))
            print("[AppModel] set_assist_sensitivity sent (\(value.rawValue))")
        } catch {
            print("[AppModel] set_assist_sensitivity send failed: \(error)")
        }
    }
    ...
    private func localMeetingTeardown() {
        ...
        meetingStartedAt = nil
        momentStatus = nil
        snapshotAudioSourceDeviceId = nil
    }
    ...
    /// The audio-source binding the server reported for the CURRENT
    /// active meeting in its last snapshot. Held so we can (re)start
    /// capture once we know our own device id — the snapshot and the
    /// `device_registered` reply race on a fresh connection, so
    /// whichever lands second triggers the resume check. Cleared when
    /// the meeting goes idle. This is the recovery path: after a
    /// crash/force-quit the meeting stays Active server-side bound to
    /// this Mac, but no `audio_source_device_changed` event fires on
    /// reconnect (the binding didn't change), so the snapshot is the
    /// only carrier of "you're still the source — resume."
    private var snapshotAudioSourceDeviceId: String?

    /// If the server's snapshot says the active meeting is still bound
    /// to THIS Mac as the audio source and we're not capturing, resume
    /// the `/audio` stream. Unlike `reconcileAudioSource`, this does
    /// NOT clear the transcript — the snapshot already delivered the
    /// current items, and this is a resume, not a fresh meeting.
    /// Idempotent: the `state == .stopped` guard makes repeat calls
    /// (snapshot + device_registered) safe.
    private func resumeBoundAudioIfNeeded() {
        // Diagnostic: log every decision so a failed resume (the
        // kill→reopen "audio didn't come back" case) is explainable
        // from `swift run` stdout instead of guessed at.
        let mine = ownDevice?.id
        let state = audioCapture.state
        guard let bound = snapshotAudioSourceDeviceId else {
            return  // no active bound meeting in the last snapshot
        }
        guard bound == mine else {
            print(
                "[AppModel] resume SKIPPED: meeting bound to device \(bound) but ours is \(mine ?? "nil") — device id didn't match (mismatch ⇒ won't resume)"
            )
            return
        }
        guard state == .stopped else {
            print("[AppModel] resume SKIPPED: already capturing (state=\(String(describing: state)))")
            return
        }
        print("[AppModel] resume: active meeting bound to us — restarting /audio capture")
        Task {
            let ok = await startAudioStream()
            print("[AppModel] resume: startAudioStream returned \(ok)")
        }
    }
    ...
    /// The user's saved quick-ask library, mirrored from the server's
    /// `quick_asks` mode items_update. UI reads this for the chat
    /// chip row + the settings editor.
    var quickAsks: [Item] {
        itemsByMode["quick_asks"] ?? []
    }

    /// Upsert one quick ask. `id` is client-minted UUID for new entries;
    /// reuse the existing id when editing. Server validates label /
    /// text lengths + the 50-asks-per-user cap; failures surface via
    /// the server's `Error` event.
    func upsertQuickAsk(id: String, label: String, text: String, position: Int32) async {
        let l = label.trimmingCharacters(in: .whitespacesAndNewlines)
        let tx = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !l.isEmpty, !tx.isEmpty, webSocket.state == .connected else { return }

        do {
            try await webSocket.send(intent: UpsertQuickAskIntent(
                id: id,
                label: l,
                text: tx,
                position: position
            ...
            print("[AppModel] upsert_quick_ask send failed: \(error)")
        }
    }

    /// Delete a saved quick ask. Idempotent server-side — unknown
    /// ids are silently dropped.
    func deleteQuickAsk(id: String) async {
        guard webSocket.state == .connected else { return }
        do {
            try await webSocket.send(intent: DeleteQuickAskIntent(id: id))
        } catch {
            print("[AppModel] delete_quick_ask send failed: \(error)")
        }
    }
    ...
    private func handle(event: TypedServerEvent) {
        // Breadcrumb for the hang watchdog: every server event is
        // applied on the main actor here, so if the main thread
        // beachballs this records what it was last doing + the item
        // counts (the growth signal for the "slows over a long
        // meeting" symptom). Cheap: a lock + string.
        watchdog.note(
            "ws event; chat=\(itemsByMode["chat"]?.count ?? 0) transcript=\(itemsByMode["transcript"]?.count ?? 0) mode=\(currentMode)"
        )
        switch event {
        case .snapshot(let payload):
            ...
            // `currentMode` is per-surface UI state — Mac does NOT
            // inherit it from the server's snapshot. Keep whatever
            // mode the user was on locally (defaults to "transcript"
            // via AppModel init). The snapshot's items still land
            // under their canonical mode key.
            displayTag = payload.displayTag
            currentMeetingId = payload.meetingId
            // Snapshot only carries the *current* mode's items; the
            // others stay empty until the user clicks them. We keep
            // them bucketed under the server-reported mode regardless
            // of our local currentMode pointer.
            itemsByMode = [payload.mode: payload.items]
            // Sensitivity defaults to Moderate when the field is
            // absent (older server build) or NULL on the meeting row.
            assistSensitivity = payload.assistSensitivity ?? .moderate
            ...
            // Recovery: if the meeting is still Active and bound to this
            // Mac, resume audio capture (it stopped when we crashed /
            // force-quit). Stash the binding and check now; if
            // `ownDevice` isn't known yet, `device_registered` will
            // re-run the check. Idle clears it so a later snapshot
            // can't spuriously resume.
            print(
                "[AppModel] snapshot: state=\(payload.meetingState) meeting=\(payload.meetingId ?? "nil") audioSource=\(payload.audioSourceDeviceId ?? "nil") ownDevice=\(ownDevice?.id ?? "nil")"
            )
            snapshotAudioSourceDeviceId =
                payload.meetingState == "active" ? payload.audioSourceDeviceId : nil
            resumeBoundAudioIfNeeded()
        case .meetingStateChanged(let state, let meetingId):
            currentMeetingId = meetingId
            if state == "active" {
                // Rule 5 state machine. Locally-initiated starts
                // always surface the overlay; remote-initiated ones
                // respect the persisted `overlayAutoShow` preference.
                // The in-flight `isLocallyStartingMeeting` flag is
                // the discriminator — see `startMeeting()`.
                let wasLocalStart = isLocallyStartingMeeting
                isLocallyStartingMeeting = false
                if wasLocalStart || settings.overlayAutoShow {
                    showOverlayWindow()
                }
            }
            if state == "idle" {
        ...
        case .deviceRegistered(let device):
            ...
            // We may have received the snapshot before knowing our own
            // device id — now that we do, re-run the resume check.
            resumeBoundAudioIfNeeded()
        ...
        case .modeChanged(let mode, _, let items):
            // `currentMode` is per-surface UI state — Mac doesn't
            // follow cross-surface view broadcasts. Keep the items
            // payload so any future local mode switch sees fresh
            // data; just don't mirror currentMode / displayTag.
        ...
        case .assistSensitivityChanged(let value):
            // Mirror the canonical value. Other surfaces (PWA / mobile)
            // can flip this during an active meeting; we treat the
            // server's broadcast as the source of truth so the local
            // picker reflects the latest choice.
            assistSensitivity = value
    ...
    /// overlay from carrying state across meetings. The quick_asks
    /// "library" (saved prompts) is per-user, not per-meeting, so
    /// it's preserved across the wipe — without that, the chip row
    /// + the Settings → Quick Asks tab go empty after every
    /// meeting-end until the user reopens the app or another
    /// surface triggers an items_update. Mirrors the PWA's
    /// equivalent fix in ws-handlers.ts.
    func clearTranscript() {
        transcriptInterim = ""
        let preservedQuickAsks = itemsByMode["quick_asks"] ?? []
        itemsByMode = preservedQuickAsks.isEmpty ? [:] : ["quick_asks": preservedQuickAsks]
        currentMode = "transcript"
        displayTag = nil
    }

    /// Switch the Mac overlay to a different mode. Purely local
    /// state — `currentMode` is per-surface UI state, not synced
    /// across surfaces. The server-side `set_mode` intent is a
    /// legacy no-op now; we don't send it.
    ...
    /// a meeting is active; the agent's reply lands as new chat-mode
    /// items via `ItemsUpdate` events.
    ///
    /// We no longer optimistically echo locally — the server emits a
    /// user-bubble + assistant-pending pair the moment the intent
    /// lands (see `ws.rs::handle_chat`) and broadcasts to every
    /// connected surface (this Mac included). Doing our OWN local
    /// echo on top would create a duplicate pair with mismatched ids
    /// that `mergeItems`'s merge-by-id can't reconcile against the
    /// server's items. So this method now just dispatches the intent;
    /// the bubbles appear from the inbound items_update.
    func sendChat(_ text: String) async {
        ...
        // Drain attachment ids BEFORE the WS send. Failed chips are
        // silently dropped from the dispatch but surfaced via a
    ...
    ///
    /// For Append modes, this matches PWA/mobile's `applyItemsUpdate`
    /// semantics: items whose id already exists in the buffer are
    /// REPLACED in place (upsert), items with new ids are APPENDED.
    /// That's what makes the optimistic chat-pending flow work — the
    /// server emits the same q_id/a_id for the pending placeholder
    /// and the final reply, and merge-by-id swaps the bubble's text
    /// + meta in place without leaving a stale placeholder behind.
