...
final class Auth0Client: NSObject {
    ...
    private let audience = Auth0Client.configured("AUTH0_API_AUDIENCE", "Auth0Audience")
        ?? "https://auris.api"
    private let redirectScheme = "auris"
    ...
    private enum Keys {
        static let refreshToken = "auris.auth.refreshToken"
        static let identitySub = "auris.auth.identity.sub"
        static let identityEmail = "auris.auth.identity.email"
        static let identityName = "auris.auth.identity.name"
        static let identityPicture = "auris.auth.identity.picture"
    }
