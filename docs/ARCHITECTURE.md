...
Two model pools, each routed through
`rig::providers::{bedrock, openai, anthropic, gemini, xai}`: the chat pool
(agent loop) is configured by `AURIS_LLM_CHAT_PROVIDER` +
`AURIS_LLM_CHAT_MODEL_ID`; the background pool (summary, moment,
artifact, wrap-up, meeting-start metadata) by
`AURIS_LLM_BACKGROUND_PROVIDER` + `AURIS_LLM_BACKGROUND_MODEL_ID`.
Recommended defaults: chat = Anthropic Opus 4.7 (1M context),
background = Anthropic Haiku 4.5.
