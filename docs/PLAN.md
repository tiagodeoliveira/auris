...
- `expand_item` end-to-end via the agent — clicking an item on any
  ...
  via `item_updated`.
- Container-shaped server: same Docker image for local dev and
  production. The single-VM deploy topology lives in the kleos repo,
  not here — kleos pulls pinned image tags from GHCR.
...
Single-VM deploy topology lives in the kleos repo (Postgres, auris,
mnemo, nginx, and scheduled backups), with image tags pulled from
GHCR. Update flow is kleos-side: `docker compose pull && up -d`.
