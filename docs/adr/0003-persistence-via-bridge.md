...
**Status:** Accepted (with Resolution 2026-05-27 — see end)
**Date:** 2026-05-02
...
- ~~Phase 1 hardware: confirm `bridge.setLocalStorage` actually persists across an Even Realities App restart (not just an in-app reload).~~ See Resolution below.

## Resolution (2026-05-27)

A real-world incident — paired-device tokens disappearing within an
hour of pairing, forcing daily re-pairs — surfaced what looked like
the worst-case scenario this ADR worried about. Investigation showed
the root cause was different:

**`bridge.setLocalStorage` does persist as the SDK docs claim.** The
data loss came from concurrent writes racing on the BLE channel that
backs the bridge. All `bridge.*` calls share one BLE link to the
EvenHub app on the phone (per the `everything-evenhub/glasses-ui`
skill's "Serialize all bridge calls, not just images — concurrent
render + storage calls can crash the connection"). `auth.ts`'s
`persistTokens` was firing four `setLocalStorage` calls inside a
`Promise.all`; some keys landed, others didn't, and the next cold
app start saw partial state.

Database evidence: paired_devices rows getting re-created within 42
minutes of the original pair — well inside the 1-hour access-token
TTL, ruling out anything on the server's refresh path.

**Fix:** commit `0d2a184` (fix(pwa): serialize bridge KV writes…)
serialized `persistTokens` / `clearStoredTokens` / `loadStoredTokens`
with sequential `await`s and added a boot-time `[auth] boot kv
presence …` log so future regressions surface in Web Inspector
without leaking token values.

**What the ADR still gets right:** `bridge.setLocalStorage` IS the
correct persistence surface — the original Decision stands. The
`localStorage` fallback is still useful for `vite dev` ergonomics
and is not a substitute on hardware.

**What it underestimated:** treating bridge calls as cheap local
operations. Storage is a BLE round-trip, not an in-process map
write — concurrency rules that apply to image uploads apply here
too.
