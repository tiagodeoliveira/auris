...
- **Single-VM deploy** — `docker-compose.deploy.yml` + `.env.deploy`
  on a Tailscale-routed VM, vs a managed Postgres + Fly.io / Render
  / Cloud Run shape. Why the simpler deploy now and what would
  push us toward managed services later.
